...
struct PostInspectionView: View {
    ...
    private var isAllChecked: Bool {
        return oilCheck && hornCheck && clutchCheck && airbagsCheck && physicalDamageCheck && tyrePressureCheck && brakesCheck && selectedImages.count == 4 && mileage != nil
    }
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ...
                Button(action: {
                    ...
                    HStack {
                        ...
                        Text(isSubmitting ? "Recording Inspection..." : (overallCheckStatus == "Verified" ? "Complete Inspection" : "Raise a Ticket?"))
                            .font(.system(size: 18, weight: .semibold, design: .default))
                            .foregroundStyle(Color.white)
                    }
                    .frame(maxWidth: .infinity)
            ...
            .navigationTitle("Post Inspection")
    ...
    private func submitInspectionAndProceed() {
        ...
        // For ticket raising, show the add ticket sheet
        if overallCheckStatus == "Raise Ticket" {
            self.showAddTicketSheet = true
            isSubmitting = false
            return
        }
        
        // For "Verified" status, record inspection and navigate to MainView
        FirebaseManager.shared.recordPostInspection(
            driverId: driverId,
            tyrePressureRemarks: tyrePressureRemarks,
            brakeRemarks: brakeRemarks,
            ...
            completion: { result in
                ...
                case .success:
                    ...
                    self.db.collection("trips").document(self.tripID).updateData([
                        ...
                        "completedAt": Date(),
                        "completedBy": driverId,
                        "endMileage": self.mileage as Any,
                        "hasIssue": false
                    ]) { error in
                        if let error = error {
                            print("Error updating trip status: \(error.localizedDescription)")
                            ...
                            // Dismiss modal and navigate to MainView
                            DispatchQueue.main.async {
                                self.isPresented = false
                                self.resetRootView(to: MainView(authVM: self.authVM))
                            }
                        }
                    }
                case .failure(let error):
