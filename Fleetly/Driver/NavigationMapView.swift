...
struct NavigationMapView: View {
    ...
    private var mapView: some View {
        ZStack {
            ...
            // Redesigned deviation alert: floating pill at the bottom
    ...
    // 1. First, let's create separate components to break up the complex expression
    ...
    // Now refactor the main filterOptionsView
    private var filterOptionsView: some View {
        Group {
            if showFilterOptions {
                ZStack {
                    // Background blur with tap gesture
                    BlurView()
                        .edgesIgnoringSafeArea(.all)
                        .onTapGesture {
                            toggleFilter(.none)
        ...
        GeometryReader { geometry in
            ZStack(alignment: .bottom) {
                mapView
                    ...
                    .onDisappear {
                        // Only stop monitoring if the trip is ended
                        if !isTripStarted {
        ...
        HStack(spacing: 25) {
            Button(action: {
                if isTripStarted {
                    ...
                    // Start trip (geofence monitoring starts automatically)
                }
        ...
        NavigationView {
            GeometryReader { geometry in
                ...
                .fullScreenCover(isPresented: $navigateToPostInspection) {
                    ...
                    // Note: Consider adding @Binding var isPresented: Bool to PostInspectionView
                    // to allow dismissal, e.g., PostInspectionView(isPresented: $navigateToPostInspection, ...)
                }
                .dismissKeyboard() // Apply keyboard dismissal at the root level
            }
        }
            ...
            GeometryReader { geometry in
                ...
                .fullScreenCover(isPresented: $navigateToPostInspection) {
                    PostInspectionView(
                        ...
                        tripID: trip.id,
                        vehicleID: trip.vehicleId,
                        isPresented: $navigateToPostInspection
                    )
                }
                .dismissKeyboard() // Apply keyboard dismissal at the root level
            }
